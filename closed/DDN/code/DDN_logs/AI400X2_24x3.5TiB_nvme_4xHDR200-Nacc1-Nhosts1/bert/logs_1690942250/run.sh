...
find_nb_files()
{
    ...
    export num_files=$(./benchmark.sh datasize --workload bert --num-accelerators ${nb_accelerator} --host-memory-in-gb ${free_mem_gb}|grep -E -o "=[0-9]+"| sed "s@=@@g"|awk '{print int($1)}')
...
{
    clush -abS "rm -rf ${exafs}/datagen_bert_\${HOSTNAME}" || return 1
    clush -abS "rm -rf ${exafs}/checkpoint_bert_\${HOSTNAME}" || return 1
}
create_files_from_each_node()
{
    ...
    clush -abS "(cd /root/storage-v0.5/ && ./benchmark.sh datagen --workload bert \
	       --num-parallel 32 \
	       --param dataset.num_files_train=${num_files} \
	       --param dataset.data_folder=${exafs}/datagen_bert_\${HOSTNAME}  \
	       ${param_storage_type} ${param_storage_root})" || return 1
    # Check the total size generated
    clush -abS "du -hc ${exafs}/datagen_bert_\${HOSTNAME}" || return 1
    # Check the number of files
    clush -abS "find ${exafs}/datagen_bert_\${HOSTNAME} -type f |wc -l" || return 1
...
run_bert()
{
    ...
    for iter in {0..4} ; do
	export logfile="result_mult_host_closed_1node_1accelerator_bert_${nb_reader_threads}readerthreads_${nb_accelerator}nbaccelerator_AI400X2_notuning_iter${iter}_PUB.log"
	clear_caches || return 1
	clush -w c017 -bS "set -o pipefail && cd /root/storage-v0.5/ && date && ./benchmark.sh run --workload bert \
		   ...
		   --param checkpoint.checkpoint_folder=${exafs}/checkpoint_bert_\${HOSTNAME} \
