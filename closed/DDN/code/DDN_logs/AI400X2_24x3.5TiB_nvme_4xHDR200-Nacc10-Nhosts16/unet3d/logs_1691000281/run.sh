...
run_unet3d()
{
    ...
    for iter in {0..4} ; do
	export logfile="result_mult_host_closed_N_unet_${nb_reader_threads}readerthreads_${nb_accelerator}nbaccelerator_AI400X2_notuning_iter${iter}_PUB.log"
	clear_caches || return 1
	clush -abS "set -o pipefail && cd /root/storage-v0.5/ && date && ./benchmark.sh run --workload unet3d \
