...
# systemd-run --unit=mycgroup --scope -p MemoryMax=$((32*1024*1024*1024)) ./single_host_closed_1_bert_PUB.sh |& tee single_host_closed_1_bert_PUB.log
...
for iter in {0..4} ; do 
    export logfile="result_single_host_closed_1_bert_${nb_reader_threads}readerthreads_${nb_compute_threads}computethreads_${nb_accelerator}nbaccelerator_AI400X2_notuning_iter${iter}_PUB.log"
    clear_caches		
